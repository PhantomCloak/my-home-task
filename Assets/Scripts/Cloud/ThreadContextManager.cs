...
public class ThreadContextManager : MonoBehaviour
{
    ...
    void Update()
    {
        while (callbacks.Count > 0)
        {
            ...
            {
                Debug.LogError("Callback returned null possible race condition");
            }
    ...
    public static SyncGenericCallback GetSynchronizeCallbackHandler(Delegate callback)
    {
        if (_instance == null)
        {
            Debug.LogWarning("Cannot find instace of ThreadContextManager please try to add scene if does not exist");
            return null;
        }

        if (callback == null)
        {
            Debug.Log("Warn: callback is null");
            return null;
        }
